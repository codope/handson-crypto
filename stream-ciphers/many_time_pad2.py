...
def probable_spaces_xor(xored_hex):
    """Determine probable spaces in the xored result."""
    probable_spaces = ''
    for i in range(0, len(xored_hex), 2):
        byte = int(xored_hex[i:i + 2], 16)
        if 65 <= byte <= 90 or 97 <= byte <= 122:  # A-Z or a-z
            probable_spaces += '20'  # ASCII for space
        else:
            probable_spaces += '00'
...
def main():
    ...
    # Determine probable spaces in each ciphertext by XORing with all other ciphertexts
    spaces_for_each_ct = []
    for i in range(len(ciphertexts)):
        probable_spaces = ''
        for j in range(len(ciphertexts)):
            if i != j:
                xored_result = xor_strings(ciphertexts[i], ciphertexts[j])
                probable_spaces = xor_strings(probable_spaces, probable_spaces_xor(xored_result))
        spaces_for_each_ct.append(probable_spaces)

    # Guess the target plaintext by XORing target ciphertext with probable spaces from each ciphertext
    target_plaintext = ''
    for i in range(len(target_ciphertext)):
        guessed_char = '.'
        for j in range(len(ciphertexts)):
            xored_result = int(xor_strings(target_ciphertext[i:i + 2], ciphertexts[j][i:i + 2]), 16)
            if int(spaces_for_each_ct[j][i:i + 2], 16) == 0x20 and (32 <= xored_result <= 126):
                guessed_char = chr(xored_result)
                break
        target_plaintext += guessed_char

    print("Guessed plaintext:", target_plaintext)
